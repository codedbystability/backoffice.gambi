...
export const getLabel = (
    ...
    if (viewMode === Views.WEEK)
        return `${moment(date).locale('tr').startOf('week').format('MMM D')} - ${moment(date)
            .endOf('week')
            .format('MMM D')}`;
    if (viewMode === Views.WORK_WEEK)
        return `${moment(date).locale('tr').startOf('week').add(1, 'day').format('MMM D')} - ${moment(date)
            .endOf('week')
            .add(-1, 'day')
            .format('MMM D')}`;
    if (viewMode === Views.AGENDA)
        return `${moment(date).locale('tr').format('L')} - ${moment(date).add(1, 'month').format('L')}`;
    return moment(date).locale('tr').format('dddd, MMM D');
};
